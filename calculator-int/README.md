...
- Subtraction returns a magnitude with a sign indicator: the absolute value is shown on the display, and the leftmost digit shows a minus sign when the result is negative.
...
| Subtract | -15 ... 15 |
| Multiply | 0 ... 225 |
| Divide | integer quotient (B = 0 guarded, shows 9999) |
...
From the Vivado Tcl Console, inside the `calculator-int/` directory:

```tcl
source scripts/create_project.tcl
```

Then: Generate Bitstream -> program the board, flip the switches, and read the display.
